...
class LobbyScreen(QtW.QWidget):
    ...
    def start_game(self):
        # Don't listen to updates anymore
        self.start_game_updater.disconnect()
        
        # Set all players to 2, indicating everybody needs to run the setup
        for player in self.Refs('connections').get():
            self.Refs(f'connections/{player}').set(2)
